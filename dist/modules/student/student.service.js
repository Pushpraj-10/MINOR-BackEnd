"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StudentService = void 0;
const prisma_1 = __importDefault(require("../../config/prisma"));
class StudentService {
    // ========== CREATE ==========
    static async createRegistration(data) {
        const existing = await prisma_1.default.studentCourseRegistration.findUnique({
            where: {
                studentId_offeringId: {
                    studentId: data.studentId,
                    offeringId: data.offeringId,
                },
            },
        });
        if (existing) {
            throw new Error("Student already registered for this offering");
        }
        const registration = await prisma_1.default.studentCourseRegistration.create({
            data: {
                studentId: data.studentId,
                offeringId: data.offeringId,
            },
        });
        return registration;
    }
    // ========== UPDATE ==========
    static async updateRegistration(id, data) {
        const existing = await prisma_1.default.studentCourseRegistration.findUnique({
            where: { id },
        });
        if (!existing) {
            throw new Error("Registration not found");
        }
        const updated = await prisma_1.default.studentCourseRegistration.update({
            where: { id },
            data,
        });
        return updated;
    }
    // ========== DELETE ==========
    static async deleteRegistration(id) {
        const existing = await prisma_1.default.studentCourseRegistration.findUnique({
            where: { id },
        });
        if (!existing) {
            throw new Error("Registration not found");
        }
        await prisma_1.default.studentCourseRegistration.delete({
            where: { id },
        });
        return { message: "Registration deleted successfully" };
    }
    // ========== ATTENDANCE ==========
    static async markAttendance(studentId, data) {
        // 1. Find QR token
        const qrToken = await prisma_1.default.qRToken.findUnique({
            where: { token: data.token },
        });
        if (!qrToken) {
            throw new Error("Invalid QR token");
        }
        // 2. Validate token time
        const now = new Date();
        if (qrToken.validUntil && qrToken.validUntil < now) {
            throw new Error("QR token expired");
        }
        if (qrToken.validFrom > now) {
            throw new Error("QR token not active yet");
        }
        // 3. Ensure student not already marked
        const existing = await prisma_1.default.attendanceRecord.findUnique({
            where: {
                sessionId_studentId: {
                    sessionId: qrToken.sessionId,
                    studentId,
                },
            },
        });
        if (existing) {
            throw new Error("Attendance already marked for this session");
        }
        // 4. Create attendance record
        const record = await prisma_1.default.attendanceRecord.create({
            data: {
                sessionId: qrToken.sessionId,
                studentId,
                status: "present",
                source: "qr",
                markedById: studentId,
            },
        });
        return record;
    }
}
exports.StudentService = StudentService;
//# sourceMappingURL=student.service.js.map